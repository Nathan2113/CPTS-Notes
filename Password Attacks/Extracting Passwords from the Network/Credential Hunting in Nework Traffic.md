...
# Pcredz
- used to extract credentials from live traffic or packet captures
...
```
```Markdown
Nathan2112@htb[/htb]$ ./Pcredz -f demo.pcapng -t -v
Pcredz 2.0.2
...
- filter by different protocols
```Markdown
http || ftp || smtp || pop || imap || snmp
```
- filter for credit card numbers (may need to update)
```Markdown
\b(?:\d[ -]?){13,19}\b
```