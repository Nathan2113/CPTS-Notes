- databases are used to keep track of your results
...
## MSF - Initiate the Database
```Markdown
# If an error gets thrown, do sudo apt update then init again
sudo msfdb init
# Then check status
sudo msfdb status
```
...
```Markdown
# Launches msfconsle and connects simultaneously
sudo msfdb run
```
  
## MSF - Reinitiate the Database
```Markdown
# Can be useful if you already have a database configured and can't change 
# the MSF username, do these commands
msfdb reinit
...
# Using the Database
## Workspaces
- segregate the different scan resuls, hostts, and extracted information by IP, subnet, network, or domain
```Markdown
msf6 > workspace
* default
...
* Target_1
```
- can do this for each tenant
  
- check workspace commands with `workspace -h`
```Markdown
msf6 > workspace -h
Usage:
...
- .xml file type is preferred for msf imports
  
## Stored Nmap Scan
```Markdown
Nathan2112@htb[/htb]$ cat Target.nmap
...
- do db_nmap instead of just nmap
```Markdown
...
# Services
- contains a table with descriptions and information on services discovered during scans
```Markdown
msf6 > services -h
...
# Credentials
- visualize credentials gathered during interactions with target host
    
    - can add credentals manually, match existing creds with port specs, add descriptions, etc
...
# Loot
- at-a-glance list of owned services and users
    
    - hash dumps (hashes, passwd, shadow, and more
