...
- can run `ps` to get the processes on the system, then `steal_token` to elevate to whatever is running that process
```Markdown
meterpreter > getuid
...
```
- 1836 is wmiprvse.exe
- from this they got N AUTHORITY\NETWORK SERVICE
    
    - basically admin rights for the web stuff
    
    - can check out IIS and go from there
    
  
# Session Handling
- can background the session, then find an exploit suggestor and run that on a session of your choosing
```Markdown
meterpreter > bg
...
- from the output above, one of the exploits they are vulnerable to is ms15_051_client_copy_image
- can then use the exploit on the session
```Markdown
...
## Dumping Hashes
- once you have NT AUTHORITY\SYSTEM, use `hashdump` or `lsa_dump_sam`
```Markdown
