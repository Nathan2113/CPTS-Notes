...
## Footprinting RDP
can see if NLA (Network Level Authentication) is enabled with Nmap
- acts as a pre-authentication step
```JavaScript
nmap -sV -sC <IP> -p3389 --script rdp*
```
```JavaScript
Nathan2112@htb[/htb]$ nmap -sV -sC 10.129.201.248 -p3389 --script rdp*
...
```
- can also use packet-trace to track individual packages
    
    - some EDRs will look for the RDP cookie (mstshash=nmap) and lock users out that way
    
```JavaScript
Nathan2112@htb[/htb]$ nmap -sV -sC 10.129.201.248 -p3389 --packet-trace --disable-arp-ping -n
...
### Security Check
```JavaScript
Nathan2112@htb[/htb]$ git clone https://github.com/CiscoCXSecurity/rdp-sec-check.git && cd rdp-sec-check
Nathan2112@htb[/htb]$ ./rdp-sec-check.pl 10.129.201.248
...
### Initiate Session
```JavaScript
Nathan2112@htb[/htb]$ xfreerdp /u:cry0l1t3 /p:"P455w0rd!" /v:10.129.201.248
...
- another component is Windows Remote Shell (WinRS) to remotely execute commands
    
    - started being enabled by default on Windows 7
    
- services like remote sessions using PowerShell and event log monitoring require WinRM, which is enabled by default starting with Windows Server 2012
...
```
```JavaScript
Nathan2112@htb[/htb]$ nmap -sV -sC 10.129.201.248 -p5985,5986 --disable-arp-ping -n
...
### Evil-WinRM
- check if remote servers can be reached via WinRM
```JavaScript
evil-winrm -i <IP> -u <user> -p <pass>
```
```JavaScript
Nathan2112@htb[/htb]$ evil-winrm -i 10.129.201.248 -u Cry0l1t3 -p P455w0rD!
...
- read/write access to almost all settings on Windows systems
- accessed via PowerShell, VBScript, or WMIC (Windows Management Instrumentation Console)
...
```
```JavaScript
Nathan2112@htb[/htb]$ impacket-wmiexec Cry0l1t3:"P455w0rD!"@10.129.201.248 "hostname"
